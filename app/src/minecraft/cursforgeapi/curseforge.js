...
function innerGet(
	...
	return new Promise((resolve, reject) => {
		if (options.query !== undefined && Object.keys(options.query).length)
			url += "&" + querystring.stringify(options.query);

		fetch(url).then(response => {
			if (response && response.status === 200) {
				if (PARSE) response.json().then(json => resolve(json));
				else response.text().then(text => resolve(conversionFunction(text)));
			} else {
				reject({id: options.identifier, name: options.name});
			}
		});
	});
}
 ...
 * @returns {Promise.<Mod, Error>} A promise containing the json object returned by the Curse API on successful 200 response.
 */
module.exports.getMod = function (identifier, name, callback) {
	let promise = innerGet(base_url + identifier, {identifier, name}, function (obj) {
		return new Mod(obj);
	});
	if (callback && typeof callback === "function")
		promise.then(callback.bind(null, null), callback);
 ...
 */
module.exports.getModFiles = function (identifier, name, callback) {
	let promise = innerGet(
